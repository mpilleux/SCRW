...
public class TemporaryFileDownloadResource extends StreamResource {
	 ...
	 *            the temp file
	 *  @param deletable
	 *            if file will be deleted at end          
	 *  
	 * @throws FileNotFoundException
	 *             the file not found exception
	 */
	public TemporaryFileDownloadResource(final Application application,
			final String fileName, final String contentType, final File tempFile, final boolean deletable )
			throws FileNotFoundException {
		
		super(new FileStreamResource(tempFile, deletable), fileName, application);
		this.filename = fileName;
		this.contentType = contentType;
	}
	...
	private static class FileStreamResource implements
		 ...
		 *             the file not found exception
		 */
		public FileStreamResource(final File fileToDownload, final boolean deletable )
				throws FileNotFoundException 
				{
			if( deletable )
				inputStream = new DeletingFileInputStream(fileToDownload);
			else
				inputStream = new NonDeletingFileInputStream(fileToDownload);
		}
